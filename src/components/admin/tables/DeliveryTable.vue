            ...
            <template v-for="delivery in dataToDisplay" :key="delivery.id">
              <tr>
                ...
                <td>
                  ...
                  <span
                    class="text-muted fw-semobold text-muted d-block fs-7"
                    >{{ delivery.customer_order_id }}</span
                  >
                </td>
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const editProfileData = ref<AllTypes.DeliveryType>(
      defaultTypes.DeliveryEmpty
    );
    const viewDeliveryData = ref<AllTypes.DeliveryType>(
      defaultTypes.DeliveryEmpty
    );
    ...
    const fetchAllAdminProfiles = async (page) => {
      const profiles = await axios
        .get(API_URL + "deliveries" + `?page=${page}`, {
